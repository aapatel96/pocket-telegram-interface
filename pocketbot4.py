...
##        print "============"
        if len(userfind.currentList.list[i].values()) == 20:
            title = userfind.currentList.list[i]["resolved_title"].encode('utf-8').strip()
            url = userfind.currentList.list[i]["resolved_url"].encode('utf-8').strip()           
            string = string +title+"\n"+url+"\n"+"\n"
...
def parsetagsadd(bot,update):
    ...
    userfind.pocketInstance.add(userfind.currentURL,tags=tagsStr)
